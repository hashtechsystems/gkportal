...
if(isset($_REQUEST['btnUpdate']))
{
	...
	if($POS=="MJ Freeway")
	{
		mysql_query("UPDATE `mj_freeway_products` SET `row`='".$arr[0]."',`col`='".$arr[1]."' WHERE id='".$ProductId."'");	
	}
	else
	if($POS=="No Pos")
	{
		mysql_query("UPDATE `tbl_products` SET `row`='".$arr[0]."',`col`='".$arr[1]."' WHERE product_id='".$ProductId."'");	
	}
                    ...
                    <select name="selProduct<?=$location;?>" id="selProduct<?=$location;?>">
                      ...
                      <?php
							...
							if($_SESSION['AssignedPOS'] == "No Pos")
   							{
								$SQL_PROD = mysql_query("SELECT product_id as Pid, product_name as PName FROM `tbl_products` WHERE customer_id = ".$_SESSION['user_id']);
							}
