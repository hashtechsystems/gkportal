...
function get_data(){
	...
	$SQL_ACCOUNT=mysql_query("SELECT * FROM `tbl_products` WHERE customer_id = ".$_SESSION['user_id']);
	$data = array();
	$cnt = 0;
	while($DATA_ACCOUNT=mysql_fetch_array($SQL_ACCOUNT))
	{
		...
		$data[$cnt]['Category'] = $DATA_ACCOUNT['product_category'];
		$data[$cnt]['Strain Name'] = $DATA_ACCOUNT['strain_name'];
		$data[$cnt]['Price'] = $DATA_ACCOUNT['price'];
