...

{
	...
	if($POS=="Treez")
	{
		...
		$gkQtySql = mysql_fetch_assoc(mysql_query("SELECT gk_inventory_quantity FROM tbl_products_treez WHERE pid='".$ProductId."'"));

		$gkQty = $gkQtySql['gk_inventory_quantity'];

		if($Qty > $gkQty){

			echo "<script>alert('Assign Quantity Should Be Less Than GK Quantity ($gkQty)')</script>";
	...
	if($POS=="No Pos")
	{
		...
		$gkQtySql = mysql_fetch_assoc(mysql_query("SELECT gk_inventory_quantity FROM tbl_products WHERE product_id='".$ProductId."'"));
...

	{
		...
		$live_qty_sql = mysql_fetch_assoc(mysql_query("SELECT live_inventory_quantity FROM tbl_products_treez WHERE pid='".$ProductId."'"));
		$live_inventory_quantity = $live_qty_sql['live_inventory_quantity'];
		
		$assign_qtySql = mysql_fetch_assoc(mysql_query("SELECT SUM(avail_qty) as sum FROM tbl_planogram p INNER JOIN tbl_terminal t ON p.terminal_id = t.id  INNER JOIN tbl_customers c ON c.id=t.customer_id WHERE p.product_id='".$ProductId."' AND c.pos_assigned = 'Treez'"));
		$assign_qty = $assign_qtySql['sum'];
		$gk_inventory_quantity = $live_inventory_quantity - $assign_qty;

		mysql_query("UPDATE `tbl_products_treez` SET `row`='".$rowStr."',`col`='".$colStr."', gk_inventory_quantity = '".$gk_inventory_quantity."' WHERE pid='".$ProductId."'");			
...

	{
		...
		$live_qty_sql = mysql_fetch_assoc(mysql_query("SELECT live_inventory_quantity FROM tbl_products WHERE product_id='".$ProductId."'"));
		$live_inventory_quantity = $live_qty_sql['live_inventory_quantity'];
		
		$assign_qtySql = mysql_fetch_assoc(mysql_query("SELECT SUM(avail_qty) as sum FROM tbl_planogram p INNER JOIN tbl_terminal t ON p.terminal_id = t.id  INNER JOIN tbl_customers c ON c.id=t.customer_id WHERE p.product_id='".$ProductId."' AND c.pos_assigned = 'No Pos'"));
		$assign_qty = $assign_qtySql['sum'];
		$gk_inventory_quantity = $live_inventory_quantity - $assign_qty;

		mysql_query("UPDATE `tbl_products` SET `row`='".$rowStr."',`col`='".$colStr."', gk_inventory_quantity = '".$gk_inventory_quantity."' WHERE product_id='".$ProductId."'");	
                      ...
                      <?php
							...
							if($_SESSION['AssignedPOS'] == "Treez")

   							{
								...
								$SQL_PROD = mysql_query("SELECT pid as Pid, product_name as PName FROM `tbl_products_treez` WHERE isDeleted = '0' AND gk_inventory_quantity > 1");
