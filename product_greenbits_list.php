...
if(isset($_REQUEST['del_product'])){
	$check = mysql_query("SELECT pl.id FROM tbl_planogram pl INNER JOIN tbl_terminal t ON t.id = pl.terminal_id INNER JOIN  tbl_customers c ON c.id = t.customer_id WHERE c.pos_assigned = 'Green Bits' AND pl.product_id = '".mysql_real_escape_string($_REQUEST['del_product'])."'");
	if(mysql_num_rows($check) > 0){
		echo "<script>alert('This product is assigned to planogram. Please remove from planogram then delete from here')</script>";
	}
	else{
		mysql_query("UPDATE tbl_greenbits_inventory SET isDeleted = '1' WHERE id = '".mysql_real_escape_string($_REQUEST['del_product'])."'");
	}
}
...
function showResult(str) {
  ...
  xmlhttp.open("GET","searchProduct_greenbits.php?q="+str,true);
  xmlhttp.send();
}
...
<div class="" id="container">
  <div class="grybox" style="max-width:98% !important; padding:1%;display: -webkit-box; ">
   ...
   <div class="row" style="max-width:99% !important; padding:1%; display: -webkit-box; margin:0;  ">
	...
	<div class="col-md-1">
	<form method="POST" action="download_product_greenbits_list.php">
	 <input type="submit" name="downloadExcel" class="search" value="Excel" />
	  </form>
	</div> 
	<div class="col-md-1">
	<form method="POST" action="UpdateProduct_GreenBits.php">
  ...
  <table cellpadding="0" cellspacing="0" width="100%" align="center" class="table">
    ...
    <?php	
	$query=mysql_query("SELECT * FROM `tbl_greenbits_inventory` WHERE isDeleted = 0 $orderBy");
	$cnt=0;
    ...
    <tr>
	  ...
	  <td class="stat"><em><? if( $data['product_orignal_image']!=""){ ?> <img src="product_images/<?php echo $data['product_orignal_image'];?>" height="50" /> <? } ?></em></td>      
	  ...
	  <td class="stat"><em><?php echo $data['product_sell_price'];?> $</em></td>       
	  <td class="stat"><em><?php echo $data['product_weight_value'];?></em></td>      
	  <td class="stat"><em><?php echo $data['product_quantity'];?></em></td>
      ...
      <td> <a href="index.php?action=edit_greenbits_product&id=<?=$data['id'];?>"><img src="images/edit-icon.jpg" width="30"  alt=""/></a></td>
	  <td>
