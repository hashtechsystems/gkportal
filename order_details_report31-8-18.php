...
<div class="" id="container" style="margin-top:20px;">
  <div class="page-header tbg">
  <div class="col-md-8"> <h1 class="">  Order Details </h1></div>
  <div class="col-md-4" style="text-align:right;">  <a href="index.php?action=order_transactions_report&page=<?=$_REQUEST['page'];?>&ipp=<?=$_REQUEST['ipp'];?>" class="greenbtn1">Previous Page</a></div>
...
?>
  <table width="100%" border="0" cellspacing="0" cellpadding="0" align="left" class="text">
	  ...
	  <table cellpadding="0" cellspacing="0" width="90%" align="center" class="table">
		...
		$query=mysql_query("SELECT op.product_id, p.$name as product, p.$price as product_price, op.quantity, op.total_amount FROM `tbl_order_products` op INNER JOIN $product_table p ON op.product_id = p.$id WHERE op.order_id = '".mysql_real_escape_string($_REQUEST['id'])."'");
		$cnt=0;$total = 0;
